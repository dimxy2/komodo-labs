        ...
        if ( j == 1 && opretlen >= len+offset-opoffset )
        {
            ...
            if ( notarized != 0 && validated != 0 )
            {
                ...
                if ( matched != 0 && *notarizedheightp > sp->NOTARIZED_HEIGHT && *notarizedheightp < height )
                {
                    ...
                    // We have sucessfully added a matched and valid notrisation here. 
                    // We can now purge the block database of all known forks and orphaned blocks, as they can never be reorged to again.
