            ...
            if ( ASSETCHAINS_SYMBOL[0] != 0 )
            {
                if ( ASSETCHAINS_REWARD == 0 )
                {
                    if ( pblock->vtx.size() == 1 && pblock->vtx[0].vout.size() == 2 && Mining_height > ASSETCHAINS_MINHEIGHT )
                    {
                        ...
                        sleep(10);
                        continue;
                    } else fprintf(stderr,"%s vouts.%d vtx.size.%d mining.%d vs %d\n",ASSETCHAINS_SYMBOL,(int32_t)pblock->vtx[0].vout.size(),(int32_t)pblock->vtx.size(),Mining_height,ASSETCHAINS_MINHEIGHT);
                }
            }
