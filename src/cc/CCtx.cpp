        ...
        if ( GetTransaction(mtx.vin[i].prevout.hash,vintx,hashBlock,false) != 0 )
        {
            ...
            else
            {
                ...
                auto findEval = [](CC *cond, struct CCVisitor _) {
                    ...
                    std::cerr << "findEval cc_typeId(cond)=" << cc_typeId(cond) << " cc_typeName=" << cc_typeName(cond) << std::endl;

                    if (cc_typeId(cond) == CC_Eval) {
                        std::cerr << "findEval found eval=" << *cond->code << std::endl;
                    }
                    if (cc_typeId(cond) == CC_Secp256k1) {
                        std::cerr << "findEval found pubkey=" << HexStr(buf2pk(cond->publicKey)) << std::endl;
                        r = true;
                    }
                ...
                if (cc) {
                    ...
                    bool out = !cc_visit(cc, visitor);
                }

                std::cerr << "visit(cc2)..." << std::endl;
                if (cc2) {
                    CPubKey pubkey;
                    CCVisitor visitor = { findEval, (uint8_t*)"", 0, &pubkey };
